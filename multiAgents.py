...
def betterEvaluationFunction(currentGameState):
    ...
    distanceToClosestActiveGhost = effectGhostDistance = 0

    if activeGhosts:
        distanceToClosestActiveGhost = min(manhattanDistance(pos, ghost.getPosition()), distanceToClosestActiveGhost)
    else: 
        distanceToClosestActiveGhost = float("inf")
    distanceToClosestActiveGhost = max(distanceToClosestActiveGhost, 5)
        
    if scaredGhosts:
        effectGhostDistance = min(manhattanDistance(pos, ghost.getPosition()), effectGhostDistance)
    else:
        effectGhostDistance = 0 

    score = 2 * currentScore - 1.5 * distanceToClosestFood - 2 * (1./distanceToClosestActiveGhost) - 2 * effectGhostDistance - 20 * numberOfCapsulesLeft - 4 * (len(foodlist))
    return score
